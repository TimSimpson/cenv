...
fn write_file(old_env: &Option<envs::Env>, new_env: &Option<envs::Env>, script_type: ScriptLanguage, file_path: &PathBuf) -> Result<()> {
    use ScriptLanguage::*;
    use std::io::Write;

    let path_sep = match script_type { Bash => ':', Dos => ';' };
    let p = path::PathUpdater::new(path_sep);

    let new_path_str = {
        let new_path_arg = match new_env {
                None => Vec::new(),
                Some(env) => vec![path_buf_to_string(&env.bin()), path_buf_to_string(&env.lib())],
            };        
        let new_path_arg_2 = new_path_arg.iter().map(|s|s.as_str()).collect::<Vec<&str>>();
        let old_path_arg = match old_env {
                None => Vec::new(),
                Some(env) => vec![path_buf_to_string(&env.bin()), path_buf_to_string(&env.lib())],
            };        
        let old_path_arg_2 = old_path_arg.iter().map(|s|s.as_str()).collect::<Vec<&str>>();
        p.update_paths(
            "PATH", 
            &new_path_arg_2,
            &old_path_arg_2,
    ...
    let comment = match script_type { Bash => "#", Dos => "REM" };
    let export = match script_type { Bash => "export", Dos => "set" };
    ...
    let cenv_name = quote(match new_env { None => "", Some(e) => e.name() })?;
    let cget_prefix_uq = match new_env { None => "", Some(e) => &path_buf_to_string(e.directory()) };
...
pub fn set(w: &world::World, managed: bool, env_name: Option<String>) -> Result<i32> {
    ...
    let new_env = match env_name {
        Some(env_name) => {
            match env_manager.get(managed, &env_name)? {
                Some(env)=> Some(env),
                None => {
                    if managed {
    ...
    write_file(&old_env, &new_env, ScriptLanguage::Dos, &w.ops().batch_file())?;
